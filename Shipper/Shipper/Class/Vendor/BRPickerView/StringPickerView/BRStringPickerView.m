...
+ (BRStringPickerView *)shared {
    
    static dispatch_once_t predicate;
    
    static BRStringPickerView *objc;
    dispatch_once(&predicate, ^{
        
        objc = [[BRStringPickerView alloc]initWithPickerMode:BRStringPickerComponentSingle];
...
- (void)handlerResultModelArrayBlock {
    ...
    if(self.resultBlock) {
        ...
        for (NSInteger i = 0; i < self.selectIndexs.count; i++) {
            ...
            [mSelectValueArr addObject:[NSString nullToString:selectValue]];
        }
