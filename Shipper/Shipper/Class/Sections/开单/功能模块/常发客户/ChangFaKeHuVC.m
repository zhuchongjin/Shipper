...
#import "XianLuXinXiVC.h"

@interface ChangFaKeHuVC ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong) CJTableView *tableView;
@property (nonatomic,strong) UITextField *tfSearch;

@end

@implementation ChangFaKeHuVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    [self setTitleName:@"客户信息"];
...
- (void)createView{
    ...
    self.tfSearch.placeholder = @"姓名/电话";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *identifier = @"ChangFaKeHuCell";
    ChangFaKeHuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[ChangFaKeHuCell alloc] initWithFlex:nil reuseIdentifier:identifier];
    ...
    cell.blcokSelelct = ^(id anyObject) {
        @strongify(self);
//        [self pop];
        XianLuXinXiVC *vc = [[XianLuXinXiVC alloc]init];
        [self pushVc:vc];
    };
    return cell;
}
